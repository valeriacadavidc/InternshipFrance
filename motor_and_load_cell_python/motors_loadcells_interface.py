...
def set_parameters(case,velocity,initial_position,final_position=None,polling_rate=1,frequency=100,cycles=None,forward_position=None, waiting_time=None):
    ...
    if case==3:
        ...
        num_samples = int((execution_time * 1.085) * frequency)
...
def plot_graph_positions(df, path,name, language='spanish'):
    ...
    plt.savefig(f'{path}\{name}_{language_suffix}_positions.png')
    # Return the Figure instance
    return f'{path}\{name}_{language_suffix}_positions.png'

def plot_graph_forces(df,path,name,language='spanish'):
    ...
    # Organiza las columnas según los números extraídos
    ...
    plt.tight_layout()
    language_suffix = 's' if language == 'spanish' else 'e'
    plt.savefig(f'{path}\{name}_{language_suffix}_forces.png')
    # Return the Figure instance
    return f'{path}\{name}_{language_suffix}_forces.png'
  

def collected_data_modification(path,name,initial_date_hour,desired_uni_force,serial_numbers_initial_positions, initial_positions,serial_numbers_initial_forces,initial_forces):
    try:
...
def collect_data(queue_obj,motor_devices_dictionary,load_cells_list, sample_time, num_samples,path, name):
